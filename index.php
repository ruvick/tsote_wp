<?php get_header(); ?>

<?php get_template_part('template-parts/header-section'); ?>

<main class="page">

	<section id="slider" class="slider">
		<div class="slider__item">
			<div class="container">
				<div class="slider__text">
					<h1>Специальная оценка
						условий труда (СОУТ)
					</h1>
					<p>
						Наша лаборатория осуществляет деятельность
						по специальной оценке условий труда
						в соответствие с ГОСТ ISO/IEC 17025-2019
					</p>
					<div class="slider__text-color"></div>
				</div>
			</div>
		</div>
		<div class="slider__item">
			<div class="container">
				<div class="slider__text">
					<h1>Специальная оценка
						условий труда (СОУТ)
					</h1>
					<p>
						Наша лаборатория осуществляет деятельность
						по специальной оценке условий труда
						в соответствие с ГОСТ ISO/IEC 17025-2019
					</p>
					<div class="slider__text-color"></div>
				</div>
			</div>
		</div>
		<div class="slider__item">
			<div class="container">
				<div class="slider__text">
					<h1>Специальная оценка
						условий труда (СОУТ)
					</h1>
					<p>
						Наша лаборатория осуществляет деятельность
						по специальной оценке условий труда
						в соответствие с ГОСТ ISO/IEC 17025-2019
					</p>
					<div class="slider__text-color"></div>
				</div>
			</div>
		</div>
	</section>

	<section id="about" class="about">
		<div class="container">
			<h2>О нашей компании</h2>
			<p>
				Наша компания оказывает услуги по специальной оценке условий труда на предприятиях любого типа мы имеем все
				необходимые сертификаты и
				разрешения на проведение специальной оценки. География нашей работы не ограничивается территорией Курской
				области, наши специалисты проведут
				выездную экспертизу в любом городе на всей территории РФ.
			</p>
		</div>
	</section>

	<section id="services" class="services">
		<div class="container">
			<div class="block__tabs tabs">
				<div class="services__title-btn d-flex">
					<h2>Наши услуги</h2>
					<nav class="services__btn-block block__nav">
						<button class="services__btn block__navitem tab__navitem active">ОХРАНА ТРУДА</button>
						<button class="services__btn block__navitem tab__navitem">ЭКОЛОГИЧЕСКАЯ БЕЗОПАСНОСТЬ</button>
					</nav>
				</div>
				<div class="block__items">

					<div class="services__row block__item tab__item active d-flex">
						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/01.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Специальная оценка
									условий труда (СОУТ)
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/02.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Оценка
									профессиональных
									рисков
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/03.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Проект предельно
									допустимых выбросов
									(ПДВ)
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/04.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Декларация о
									воздействии на
									окружающую среду
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/05.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Расчет нормативов
									допустимых выбросов
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/06.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Инвентаризация
									источников выбросов
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/07.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Проект мероприятий
									по уменьшению
									выбросов при НМУ
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/08.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Проект нормативов
									образования отходов
									и лимитов на их
									размещение (ПНООЛР)
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/09.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Инвентаризация
									отходов производства
									и потребления
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/10.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Постановка на учет
									предприятий негативно
									воздействующих на
									окружающую среду
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/11.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Разработка проектов
									санитарно-защитных
									зон (СЗЗ)
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/12.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Разработка программ
									производственного
									экологического
									контроля (ПЭК)
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/13.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Проведение
									производственного
									экологического
									контроля
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/14.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Проведение
									инструментальных
									замеров собственной
									лабораторией
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/01.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>Специальная оценка
									условий труда (СОУТ)
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>
					</div>

					<div class="services__row block__item tab__item d-flex">
						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/04.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Декларация о
									воздействии на
									окружающую среду
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/05.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Расчет нормативов
									допустимых выбросов
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/06.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Инвентаризация
									источников выбросов
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/07.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Проект мероприятий
									по уменьшению
									выбросов при НМУ
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/08.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Проект нормативов
									образования отходов
									и лимитов на их
									размещение (ПНООЛР)
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/09.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Инвентаризация
									отходов производства
									и потребления
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/10.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Постановка на учет
									предприятий негативно
									воздействующих на
									окружающую среду
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/11.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Разработка проектов
									санитарно-защитных
									зон (СЗЗ)
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/12.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Разработка программ
									производственного
									экологического
									контроля (ПЭК)
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/13.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Проведение
									производственного
									экологического
									контроля
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/14.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>
									Проведение
									инструментальных
									замеров собственной
									лабораторией
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>

						<div class="card__body d-flex">
							<div class="card__img">
								<img src="<?php echo get_template_directory_uri(); ?>/img/services/01.jpg" alt="">
							</div>
							<div class="card__desc">
								<h4>Специальная оценка
									условий труда (СОУТ)
								</h4>
								<a href="#" class="card__link">ПОДРОБНЕЕ</a>
							</div>
						</div>
					</div>

				</div>
			</div>
		</div>
	</section>

	<section id="clients" class="clients">
		<div class="container">
			<h2>Наши клиенты</h2>
			<div class="clients__slider">

				<div class="clients__slider-item">
					<img src="<?php echo get_template_directory_uri(); ?>/img/clients/01.png" alt="">
				</div>

				<div class="clients__slider-item">
					<img src="<?php echo get_template_directory_uri(); ?>/img/clients/02.png" alt="">
				</div>

				<div class="clients__slider-item">
					<img src="<?php echo get_template_directory_uri(); ?>/img/clients/03.png" alt="">
				</div>

				<div class="clients__slider-item">
					<img src="<?php echo get_template_directory_uri(); ?>/img/clients/04.png" alt="">
				</div>

				<div class="clients__slider-item">
					<img src="<?php echo get_template_directory_uri(); ?>/img/clients/05.png" alt="">
				</div>

				<div class="clients__slider-item">
					<img src="<?php echo get_template_directory_uri(); ?>/img/clients/06.png" alt="">
				</div>

				<div class="clients__slider-item">
					<img src="<?php echo get_template_directory_uri(); ?>/img/clients/07.png" alt="">
				</div>

				<div class="clients__slider-item">
					<img src="<?php echo get_template_directory_uri(); ?>/img/clients/08.png" alt="">
				</div>

				<div class="clients__slider-item">
					<img src="<?php echo get_template_directory_uri(); ?>/img/clients/09.png" alt="">
				</div>

				<div class="clients__slider-item">
					<img src="<?php echo get_template_directory_uri(); ?>/img/clients/10.png" alt="">
				</div>

				<div class="clients__slider-item">
					<img src="<?php echo get_template_directory_uri(); ?>/img/clients/11.png" alt="">
				</div>

				<div class="clients__slider-item">
					<img src="<?php echo get_template_directory_uri(); ?>/img/clients/12.png" alt="">
				</div>

			</div>
		</div>
	</section>
</main>

<?php get_footer(); ?>